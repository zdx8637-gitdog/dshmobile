...
| 能力 | 状态 |
| :-- | :-- |
| 一条命令安装的 DSH bundle 插件（npm：`@zdx8637/dshmobile-bridge`） | ✅ beta.7，免补丁、跨平台、DSH 升级免疫 |
...
| 安全审计闭环（v0.1.3 + relay 三修复）：授权码 relay 域名绑定防令牌窃取、WS 畸形参数防崩溃、连接即校验设备归属、requestId 复合键防错投；生产实测通过 | ✅ |
| GitHub 仓库（封面/截图/简介/topics）+ npm 发布 + awesome-dsh-plugin 收录（已合并） | ✅ |
...
3. **E2EE（安全审计后提前）** —— 控制面起步：relay 只知道「谁连谁、多大、何时」，
   不知道内容。目标安全模型：**服务器负责连接，但不需被信任。**
   完成后自建 relay 的心理门槛大幅下降，与 relay 文档互相成就。
4. **安全硬化（近程，便宜且高信任）** —— 全端登出（一键踢下线）、陌生设备登录提示、
   在线连接可见；Workspace 为边界的文件访问（拒绝 `../../` 路径穿越）。
5. **界面美化** —— 插件 Web 面板 + Android 端 UI 统一视觉升级：主题一致性、
   间距/排版/暗色模式打磨、空态与错误态、动效收尾（不改动功能行为）。
6. **断线恢复打磨** —— 自动重连、会话状态恢复、事件续传、请求去重；
   用户只看到「Office-PC 正在重连… → 已连接」。
7. **Push（前台服务保活起步）** —— 无 GMS 机型用「前台服务 + 常驻通知」的 80 分方案，
   后续再评估厂商通道（华为/小米/OPPO）。
8. **Relay 可运营性** —— 容量监控、多实例、优雅降级；自建 relay 文档按
