...
`已配对 · 要求加密（e700dc966ca4…）`（绿）/ `未配对 · 要求加密`（灰）/ `已配对 · 允许明文`（琥珀），
右侧按钮在「允许明文」与「要求加密」间切换（新面板动作 `e2eePolicy`）：

- 「允许明文」→ 写 `e2ee-policy.json {require:false}` + 清 `pinnedPeer`（**解除配对**）+ 重启桥使其生效；
- 「要求加密」→ 写 `{require:true}` + 重启桥（保留本机身份，手机可重新扫码配对）。
...
1. **面板状态行已补齐**（见 §6）：宿主 `/state` 增 `e2eeRequire/e2eePinned/e2eePeerKeyId`，面板 ② 卡显示状态点+文案+切换按钮（`e2eePolicy` 动作）；需要**重启一次 DSH** 才能加载新宿主与客户端产物。
