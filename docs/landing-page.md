...
| j | **APK 0.2.21 上线（默认端到端加密）** | ✅ **已上线（2026-09-23 14:2x）**：`versionCode 32 / versionName 0.2.21`，`DSH-Mobile-0.2.21.apk` sha256 前 32 `3723cebde365867c9746da420c6eb5b5`、13,633,385 字节（服务器与本地一致）；`latest.json`/`index.html` 兜底同步 0.2.21；本地 22 项 + 线上下载链路 12 项 + 二维码 + 单页配对 6 组全绿；已镜像回 `dshmobile-landing\site\` | 已完成。内容是"默认 E2EE + 设备行两态徽标 + 三选项弹框"（详见 `docs/e2ee-default-on.md`）。**发布顺序**：桥 beta.25 的强制只对 `appVersion ≥ 0.2.21` 的客户端生效，因此本版铺开后再发 beta.25 才安全 |
| i | **APK 0.2.20 上线（会话列表可信刷新/动效）** | ✅ **已上线（2026-09-23 09:1x）**：`versionCode 31 / versionName 0.2.20`，`DSH-Mobile-0.2.20.apk` sha256 前 32 `3089005370cc5527341f26f7a562c7d2`、13,617,001 字节（服务器与本地一致）；`latest.json`/`index.html` 兜底版本同步 0.2.20；本地 22 项 + 线上下载链路 12 项 + 二维码 + 单页配对 6 组全绿；已镜像回 `dshmobile-landing\site\` | 已完成。内容是"只有主动刷新才显示刷新圈（拿到响应才结束）、失败明说旧快照时间、数据时间常显、黄/绿点出现动效、新建会话悬浮"（详见 `docs/session-list-refresh.md`） |
| h | **APK 0.2.19 上线（刷新失败可见化）** | ✅ **已上线（2026-09-20 20:1x）**：`versionCode 30 / versionName 0.2.19`，`DSH-Mobile-0.2.19.apk` sha256 前 32 `99bd78cec374250560c5982bc1aa527b`、13,600,617 字节（服务器与本地一致）；`latest.json`/`index.html` 兜底版本同步 0.2.19；本地 22 项 + 线上下载链路 12 项 + 二维码 + 单页配对 6 组全绿；已镜像回 `dshmobile-landing\site\` | 已完成。内容是"刷新会话失败不再静默"（`DEVICE_OFFLINE` 等提示直接告知原因），配合桥单实例保护（`docs/bridge-singleton.md`） |
| g | **APK 0.2.18 上线（E2EE 自愈）** | ✅ **已上线（2026-09-20 19:1x）**：`versionCode 29 / versionName 0.2.18`，`DSH-Mobile-0.2.18.apk` sha256 前 32 `65db623d50edf75b4856bf04f7380498`、13,600,617 字节，服务器与本地一致；`latest.json`/`index.html` 兜底版本同步为 0.2.18；线上 **22 项 + 下载链路 12 项 + 二维码 + 单页配对 6 组** 全绿 | 已完成。内容是"对端安全身份变化时自动丢过期 pin 并回退明文"；注意措辞：**触发症状**是用户反馈的"升级后刷新不出会话"，而"必须手动「取消加密」才能刷新"是**推断、用户并未执行**（2026-09-20 晚更正，见 `D:\p\WORKFLOW.md` §6/§7.0 第 3 项） |
