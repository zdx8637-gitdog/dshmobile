...
const ws = new WebSocket(`${WS}?clientId=diag-client&targetDeviceId=${device.id}`, ["bearer", token]);
ws.on("close", (code, reason) => console.error("WS CLOSED:", code, String(reason)));
ws.on("error", (e) => console.error("WS ERROR:", e.message));
// 先挂监听再等 open：relay 的 device.status 帧在 upgrade 后立即发送，晚挂会丢
const first = new Promise((r) => { const h = (raw) => { const e = JSON.parse(raw.toString()); if (e.type === "device.status") { ws.off("message", h); r(e); } }; ws.on("message", h); });
await new Promise((r, j) => { ws.on("open", r); ws.on("error", j); });
const statusFrame = await Promise.race([
  first,
  new Promise((_, j) => setTimeout(() => j(new Error("device.status timeout")), 8000)),
]);
console.log("ws connected, bridge online:", statusFrame.payload?.online);
